...
"@
    ...
    try {
        ...
        $null = Invoke-WslRetry "DOCKER_BUILDKIT=1 docker build --progress=plain --network=host -t ${ImageName}:base -f '$WslBuildDir/Dockerfile' '$WslBuildDir'"
        ...
        $null = Invoke-WslRetry "DOCKER_BUILDKIT=1 docker build --progress=plain --network=host -t ${ImageName}:latest --build-arg BASE_IMAGE=${ImageName}:base -f '$WslToolsDockerfile' '$WslToolsContext'"
    ...
    try {
        ...
        $null = Invoke-WslRetry "DOCKER_BUILDKIT=1 docker build --progress=plain --network=host -t ${ImageName}:base -f '$($WslBuildContext.WslContextPath)/Dockerfile' '$($WslBuildContext.WslContextPath)'"
        Write-Host "  Base image built: ${ImageName}:base" -ForegroundColor Green
